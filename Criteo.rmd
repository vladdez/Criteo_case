...
```
```{r}
df_rev %>% ungroup() %>% summarize(mean = mean(Revenue), median = median(Revenue), min = min(Revenue),max = max(Revenue))
...
**Recommendations:**
  ...
  2. Focus on Android apps in smartphones.
  3. Make on Desktop advertising the main target of campaigns.

# 3. Apps VS Mobile Web
## 3.1 problems with app
...
Excluding Android app data we can observe that Summer drop of sales was not caused by troubles with Android app. We stills observing this drop even exclyding app data.
...
```
There is a revenue by 4.8 higher € through Android app in comparison to mobile web.
...
**Recommendations:** 
  ...
  1. There were a technical problems with Android applications in summer months company leading to no information about sales and product. Company should solve this technical problems.
  2. Company will obtain more revenue if focus on promotion of its application as soon as it generates almost 5 times more revenue than mobile web and by 16% more visits of product page.
  3. However there us no listing and basketing options in application. There is a need in additional investigation about their usefulness for increase of revenue.
