...
df <- fread(wf) %>% mutate(`Existing client` = as.factor(`Existing client`),
...
The purchasing spans from 9 o'clock to 23 o'clock. The most popular time for sales id 20 o'clock.
...
```
While product visiting, basketing and purchasing happens mostly at 20 o'clock, listing is uniformly distrubted across day with maximum at 16 o'clock. 
...
df  %>% filter(`Event type` == 'Sales', `Browser family` != 'NULL', `Browser family` != 'other') %>%
  ...
  ggplot(aes(y = Usage, x = reorder(`Browser family`, desc(Usage)), label = Usage , fill = reorder(`Browser family`, desc(Usage)))) + 
  geom_col() +
  geom_text( position = position_dodge(width = 0.9), vjust = -0.5) +
  ggtitle('Distribution of browser families') + 
  labs(x = "Number of sales", fill = 'Browser family')  + 
  theme(plot.title = element_text(hjust = 0.5))
```
The majority of actions happens through Chrome, Safari and Firefox,  
...
df %>% 
ggplot(aes(x = `Existing client`)) + geom_histogram(stat="count") #+ facet_grid(~Device_source)
```
